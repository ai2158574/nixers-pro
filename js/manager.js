...
/* ============================================================
   MANAGER MODE OVERRIDES (policy v1)
   ============================================================ */
(function managerModeBootstrap(){
  const MANAGER_DENY = 'Only admin can take this action';
  ...
  function deny(msg=MANAGER_DENY){ toast(msg, 'warn'); }

  // Switch session to first manager account for manager dashboard
  const firstManager = DB.users.find(u => u.role === 'manager');
  if (firstManager) currentUser = firstManager;

  const wrapAdminOnly = (fnName, msg=MANAGER_DENY) => {
    const orig = window[fnName];
    if (typeof orig !== 'function') return;
    window[fnName] = function(...args){
      if (isManager()) return deny(msg);
      return orig.apply(this, args);
    };
  };

  // Hard guards for destructive/high-authority actions
  ['deleteUser','deletePost','deleteSite','deleteProject','deleteClient','deleteTicket','deleteHoliday','addHoliday','processPayroll','openRoleModal','saveRole','deleteRole'].forEach(name => wrapAdminOnly(name));
  wrapAdminOnly('impersonateUser', 'Managers cannot impersonate users');

  // Guard leave approvals (manager can approve workers only)
  if (typeof decideLeave === 'function') {
    const _decideLeave = decideLeave;
    window.decideLeave = function(leaveId, decision){
      if (!isManager()) return _decideLeave(leaveId, decision);
      const l = DB.leaveRequests.find(x => x.id === leaveId);
      const target = userById(l?.userId);
      if (!target || target.role !== 'worker' || !managerUserIds().has(target.id)) return deny('You can only decide leave for your team workers');
      return _decideLeave(leaveId, decision);
    };
  }

  // Guard timesheet decisions (manager can approve own team workers)
  if (typeof decideTimesheet === 'function') {
    const _decideTimesheet = decideTimesheet;
    window.decideTimesheet = function(id, decision){
      if (!isManager()) return _decideTimesheet(id, decision);
      const t = DB.timesheets.find(x => x.id === id);
      const target = userById(t?.userId);
      if (!target || target.role !== 'worker' || !managerUserIds().has(target.id)) return deny('You can only decide timesheets for your team workers');
      return _decideTimesheet(id, decision);
    };
  }

  // Guard profile edits: manager edits worker fields only, cannot set admin/manager role
  if (typeof savePMUser === 'function') {
    const _savePMUser = savePMUser;
    window.savePMUser = function(userId){
      if (isManager()) {
        if (userId) {
          const target = userById(userId);
          if (!target || target.role !== 'worker') return deny('Managers can only edit worker profiles');
        }
        const pmRole = $('pmRole');
        if (pmRole) pmRole.value = 'worker';
      }
      return _savePMUser(userId);
    };
  }

  if (typeof openProfileModal === 'function') {
    const _openProfileModal = openProfileModal;
    window.openProfileModal = function(userId=null, tab='pmEdit'){
      _openProfileModal(userId, tab);
      if (!isManager()) return;
      const target = userId ? userById(userId) : null;
  ...
  // Scope filters for data-heavy modules
  if (typeof filterAndUpdateTimesheets === 'function') {
    const _renderTimesheetTable = renderTimesheetTable;
    window.renderTimesheetTable = function(){
      if (!isManager()) return _renderTimesheetTable();
      const allowed = managerUserIds();
      const original = DB.timesheets;
      DB.timesheets = original.filter(t => allowed.has(t.userId));
      try { return _renderTimesheetTable(); }
      finally { DB.timesheets = original; }
    };
  }

  if (typeof filterAndUpdatePayroll === 'function') {
    const _filterAndUpdatePayroll = filterAndUpdatePayroll;
    window.filterAndUpdatePayroll = function(){
      if (!isManager()) return _filterAndUpdatePayroll();
      const allowed = managerUserIds();
      const original = DB.payroll;
      DB.payroll = original.filter(p => allowed.has(p.userId));
      try { return _filterAndUpdatePayroll(); }
      finally { DB.payroll = original; }
    };
  }

  if (typeof filterAndUpdateAuditLog === 'function') {
    const _filterAndUpdateAuditLog = filterAndUpdateAuditLog;
    window.filterAndUpdateAuditLog = function(){
      if (!isManager()) return _filterAndUpdateAuditLog();
      const allowed = managerUserIds();
      const original = DB.auditLog;
      DB.auditLog = original.filter(l => allowed.has(l.userId));
      try { return _filterAndUpdateAuditLog(); }
      finally { DB.auditLog = original; }
    };
  }

  // Add sentBy to email logs for ownership filtering
  if (typeof sendEmail === 'function') {
    const _sendEmail = sendEmail;
    window.sendEmail = function(to, subject, template){
      const res = _sendEmail(to, subject, template);
      const entry = DB.emailLog[0];
      if (entry && !entry.sentBy) entry.sentBy = currentUser?.id || null;
      return res;
    };
  }

  if (typeof filterAndUpdateEmailLog === 'function') {
    const _filterAndUpdateEmailLog = filterAndUpdateEmailLog;
    window.filterAndUpdateEmailLog = function(){
      if (!isManager()) return _filterAndUpdateEmailLog();
      const original = DB.emailLog;
      DB.emailLog = original.filter(e => (e.sentBy || null) === currentUser?.id);
      try { return _filterAndUpdateEmailLog(); }
      finally { DB.emailLog = original; }
    };
  }

  // Restrict showPage entry points
  if (typeof showPage === 'function') {
    const _showPage = showPage;
    window.showPage = function(pageKey){
      if (isManager() && pageKey === 'rbac') {
        deny('Access denied');
        return _showPage('dashboard');
      }
      return _showPage(pageKey);
    };
  }

  function lockGeneralSettingsFields(){
    ['sysName','sysTz','sysDateFmt','sysCurrency','currencyPosition'].forEach(id => {
      const el = $(id);
      if (!el) return;
      el.disabled = true;
      el.classList.add('locked');
      if (!el.parentElement?.querySelector('.manager-lock-note')) {
        const note = document.createElement('div');
        note.className = 'manager-lock-note';
        note.textContent = 'Only admin can perform this action.';
        el.parentElement?.appendChild(note);
      }
    });
  }

  function lockManagerUI(){
    if (!isManager()) return;
    document.body.classList.add('manager-mode');

    // Hide RBAC navigation
    document.querySelector('[data-page="rbac"]')?.remove();

    // Hide restricted settings tabs
    ['emailjs','company','leave-policy','data'].forEach(tab => {
      const btn = document.querySelector(`[data-settab="${tab}"]`);
      if (btn) btn.classList.add('manager-hidden');
    });

    // Hide explicit destructive buttons
    ['bulkDelete','addHolidayBtn','prProcess','auditExport'].forEach(id => {
      const el = $(id); if (el) el.classList.add('manager-hidden');
    });

    // Email center restrictions
    document.querySelector('[data-etab="templates"]')?.classList.add('manager-hidden');
    const bulkTargets = $('bulkEmailTargets');
    if (bulkTargets) {
      [...bulkTargets.querySelectorAll('label')].forEach(label => {
        const val = label.querySelector('input')?.value;
        if (val === 'all' || val === 'manager') label.remove();
      });
    }

    // Lock general settings fields
    lockGeneralSettingsFields();

    // Disable save settings for locked tabs context (manager can still save allowed tabs)
    const saveBtn = $('saveSettingsBtn');
    if (saveBtn) {
      saveBtn.addEventListener('click', () => {
        if (['emailjs','company','leave-policy','data'].some(tab => document.querySelector(`[data-settab="${tab}"].active`))) {
          deny();
        }
      });
    }

    // Remove delete buttons in dynamic tables by onclick name
    const stripDeleteButtons = () => {
      ['deleteUser','deletePost','deleteSite','deleteProject','deleteClient','deleteTicket','deleteHoliday'].forEach(fn => {
        document.querySelectorAll(`button[onclick*="${fn}("]`).forEach(btn => btn.remove());
      });
      // remove impersonate action
      document.querySelectorAll('button[onclick*="impersonateUser("]').forEach(btn => btn.remove());
    };

    stripDeleteButtons();
    const mo = new MutationObserver(stripDeleteButtons);
    mo.observe(document.body, { childList:true, subtree:true });
  }



// Hazard tab removed in manager.html; prevent null-element errors from legacy calls
if (typeof filterAndUpdateHazards === 'function') {
  const _filterAndUpdateHazards = filterAndUpdateHazards;
  window.filterAndUpdateHazards = function(){
    if (!document.getElementById('hazardStats') || !document.getElementById('hazTbody')) return;
    return _filterAndUpdateHazards();
  };
}
  ...
  // run once in case app already initialized
  setTimeout(lockManagerUI, 400);
})();
